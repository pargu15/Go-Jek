...
public class ProductsSelectionActions {
	...
	AddRemoveProductsRepository addProductsRepository = PageFactory.initElements(DriverInitiation.getDriver(),
			AddRemoveProductsRepository.class);
	...
	public void DirectToHeadPhonesSelectionScreen() throws Exception {
		...
		try {
			action.moveToElement(addProductsRepository.oCategoryink)
					...
					.moveToElement(addProductsRepository.oElectronicsLink).click().build().perform();
